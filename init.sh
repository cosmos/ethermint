...
if [[ $1 == "pending" ]]; then
    echo "pending mode on; block times will be set to ~60s."
    ...
    sed -i 's/timeout_commit = "5s"/timeout_commit = "50s"/g' $HOME/.ethermintd/config/config.toml
fi
...
ethermintcli rest-server --laddr "tcp://localhost:8545" --unlock-key $KEY --chain-id $CHAINID --trace > cli.log &
