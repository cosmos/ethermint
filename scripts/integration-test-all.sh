...
init_func() {
    ...
    "$PWD"/build/ethermintd keys add $KEY"$i" --home "$DATA_DIR$i" --keyring-backend test --no-backup 
    ...
    "$PWD"/build/ethermintd gentx --name $KEY"$i" --keyring-backend test --home "$DATA_DIR$i"
