...
```bash
aragond start --pruning=nothing --rpc.unsafe --log_level "main:info,state:info,mempool:info"
```
...
```bash
aragoncli rest-server --laddr "tcp://localhost:8545" --unlock-key mykey --chain-id 1
```
...
```bash
aragoncli keys unsafe-export-eth-key mykey
```
