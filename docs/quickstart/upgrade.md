...
First, stop your instance of `aragond`. Next, upgrade the software:
...
To upgrade the genesis file, you can either fetch it from a trusted source or export it locally using the `aragond export` command.
...
```bash
aragond migrate [target-version] [/path/to/genesis.json] --chain-id=<new_chain_id> --genesis-time=<yyyy-mm-ddThh:mm:ssZ>
```
