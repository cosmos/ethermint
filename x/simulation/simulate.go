...
func initChain(
	r *rand.Rand, params simulation.Params, accounts []simulation.Account, app *baseapp.BaseApp,
...
func SimulateFromSeed(
	...
	accs := RandomAccounts(params.NumKeys)
	eventStats := simulation.NewEventStats()
	...
	operationQueue := simulation.NewOperationQueue()
	...
	for height := config.InitialBlockHeight; height < config.NumBlocks+config.InitialBlockHeight && !stopEarly; height++ {
		...
		logWriter.AddEntry(simulation.BeginBlockEntry(int64(height)))
		...
		logWriter.AddEntry(simulation.EndBlockEntry(int64(height)))
	...
	return func(
		...
		for i := 0; i < blocksize; i++ {
			...
			if !config.Lean || opMsg.OK {
				logWriter.AddEntry(simulation.MsgEntry(header.Height, int64(i), opMsg))
			}
...
func runQueuedOperations(queueOps map[int][]simulation.Operation,
	...
	ctx sdk.Context, accounts []simulation.Account, logWriter simulation.LogWriter,
	...
	for i := 0; i < numOpsRan; i++ {
		...
		opMsg.LogEvent(event)
		if !lean || opMsg.OK {
			logWriter.AddEntry(simulation.QueuedMsgEntry(int64(height), opMsg))
		}
		if err != nil {
			logWriter.PrintLogs()
			tb.FailNow()
...
func runQueuedTimeOperations(queueOps []simulation.FutureOperation,
	...
	logWriter simulation.LogWriter, event func(route, op, evResult string),
