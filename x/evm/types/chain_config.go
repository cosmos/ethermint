...
//
// NOTE 1: Since empty Int (i.e with a nil big.Int value) are parsed to zero, we need to manually
// specify
// NOTE 2: This type is not a configurable Param since the SDK does not allow for validation against
// previous stored parameter values or current block height. In order to update the values, a
// hard-fork upgrade must be done.
