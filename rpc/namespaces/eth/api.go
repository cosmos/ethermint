...
func (api *PublicEthereumAPI) SendTransaction(args rpctypes.SendTxArgs) (common.Hash, error) {
	...
	info, err := api.clientCtx.Keybase.GetByAddress(sdk.AccAddress(args.From.Bytes()))
	if err != nil {
	...
	// Sign transaction
	if err := tx.Sign(api.chainIDEpoch, info.GetName(), "", api.clientCtx.Keybase.Sign); err != nil {
